...
import 'package:rice/src/personal_restaurants/personal_restaurants_screen.dart';
...
class PersonalRestaurantsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final bloc = PersonalRestaurantsBloc(riceRepository: context.read<RiceRepository>());

    final PersonalRestaurantsPageArguments args =
        ModalRoute.of(context)!.settings.arguments as PersonalRestaurantsPageArguments;

    return Scaffold(
      appBar: ScreenBar(
        Text('${args.name}'.toUpperCase(), style: Theme.of(context).textTheme.headline2, overflow: TextOverflow.ellipsis),
        isBackIcon: true,
      ),
